...
class Logger {
    constructor(filePath = null, time = false) {
    ...
    _log(fn, key, ...args) {
        ...
        if (this.filePath) {
            const date = new Date();
            const log = `${this.time ? `[${date.getHours()}:${date.getMinutes()}:${date.getSeconds}]` : ``}[${key.toUpperCase()}]: ${args.slice(1).join(' ')}\n`;

            appendFileSync(`${this.filePath}/${date.getDate()}.${date.getMonth()+1}.${date.getFullYear()}.txt`, log);
        }
    }
}
